...
def left_join(df1: pd.DataFrame, df2: pd.DataFrame, column: str) -> pd.DataFrame:
    return df1.merge(df2, on = column, how = 'left').rename(columns = {'id': f'{column}_id'})

def normalize(df: pd.DataFrame, column: str) -> pd.DataFrame:
    return df.filter(items = [column]).pipe(drop_duplicates).pipe(create_id_column)
